...
public class TestHttpPool {
    ...
    // 设置header信息
    private static final Header[] headers = HttpHeader.init().userAgent("Mozilla/5.0").from("http://blog.csdn.net/newest.html").build();
    ...
    //多线程get请求
    public static void testMultiGet(HttpApacheConfig cfg, int count) throws Exception {
        try {
    ...
    //多线程下载
    public static void testMultiDown(HttpApacheConfig cfg, int count) throws Exception {
        try {
    ...
    static class GetRunnable implements Runnable {
        ...
        private HttpApacheConfig config = null;
        private FileOutputStream out = null;
        private String url = null;

        public GetRunnable(CountDownLatch countDownLatch, HttpApacheConfig config, String url) {
            this(countDownLatch, config, url, null);
        }

        public GetRunnable(CountDownLatch countDownLatch, HttpApacheConfig config, String url, FileOutputStream out) {
     ...
     * @throws Exception
     */
    private static void testByPool(int getCount, int downCount) throws Exception {
        long start = System.currentTimeMillis();

        TemplateHttpClientBuilder hcb = TemplateHttpClientBuilder.init().pool(100, 10).timeout(10000).ssl();
        if (getCount > 0) {
            HttpApacheConfig cfg3 = HttpApacheConfig.init().client(hcb.build()).headers(headers);//使用一个client对象
            testMultiGet(cfg3, getCount);
        }
        if (downCount > 0) {
            HttpApacheConfig cfg4 = HttpApacheConfig.init().client(hcb.build());
     ...
     * @throws Exception
     */
    public static void testquickConcurrent() throws Exception {
        //---------------------------
        //---  期望结果：
        ...
        HttpClient client = TemplateHttpClientBuilder.init().pool(100, 20).timeout(10000).build();//最多创建20个http链接
        final HttpApacheConfig cfg = HttpApacheConfig.init().client(client).headers(headers);//为每次请求创建一个实例化对象
        for (int i = 0; i < 100; i++) {
            new Thread(new Runnable() {
                public void run() {
                    try {
                        int idx = ((int) (Math.random() * 10)) % 5;
                        Http.apache.get(cfg.url(urls[idx]));
