...
public class HttpClientTest {
    ...
    public static void testOne() throws Exception {
        ...
        //简单调用
        String resp = Http.apache.get(config.url(url));
        ...
        //设置header信息
        Header[] headers = HttpHeader.init().userAgent("Mozilla/5.0").build();
        //执行请求
        resp = Http.apache.get(config.headers(headers));
        System.out.println("请求结果内容长度：" + resp.length());

        System.out.println("\n#################################\n");

        System.out.println("--------代理设置（绕过证书验证）-------");
        HttpClient client = TemplateHttpClientBuilder.init().timeout(10000).proxy("127.0.0.1", 8087).ssl().build();//采用默认方式（绕过证书验证）
        //执行请求
        resp = Http.apache.get(config.client(client));
        System.out.println("请求结果内容长度：" + resp.length());
...
//		System.out.println("请求结果内容长度："+ resp.length());
        try {
            ...
            Http.apache.down(HttpApacheConfig.init().url(url).out(out));
            out.flush();
            out.close();
            System.out.println("--------下载测试+代理-------");

            out = new FileOutputStream(new File("d://aaa//001.png"));
            Http.apache.down(HttpApacheConfig.init().client(client).url(url).out(out));
            out.flush();
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static void testMutilTask() throws Exception {
        // URL列表数组
        String[] urls = {
...
//				"http://blog.csdn.net/xiaoxian8023/article/details/43312929", 
        ...
        // 设置header信息
        Header[] headers = HttpHeader.init().userAgent("Mozilla/5.0").from("http://blog.csdn.net/newest.html").build();
        HttpClient client = TemplateHttpClientBuilder.init().timeout(10000).proxy("127.0.0.1", 8087).ssl().build();//采用默认方式（绕过证书验证）
        ...
        try {
            ...
            for (int i = 0; i < pagecount * 10; i++) {
                ...
                //启动线程抓取
                executors.execute(new GetRunnable(countDownLatch).setConfig(HttpApacheConfig.init().headers(headers).url(urls[i % pagecount])));
                executors.execute(new GetRunnable(countDownLatch).setConfig(HttpApacheConfig.init().client(client).headers(headers).url(imgurls[i % 2]).out(out)));
            }
    ...
    static class GetRunnable implements Runnable {
        ...
        public GetRunnable setConfig(HttpApacheConfig config) {
            this.config = config;
            return this;
        }
